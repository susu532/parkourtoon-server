...
const blockKey = (bx: number, by: number, bz: number) =>
  (bx & 0x7ff) | ((bz & 0x7ff) << 11) | ((by & 0x3ff) << 22);
...
export function tick(ctx: GameContext, delta: number) {
  ...
  const checkLineOfSight = (p1: any, p2: any) => {
    ...
    for (let i = 1; i <= steps; i++) {
      ...
      const z = p1.z + dz * t;
      if (
        isSolidBlock(fastGetBlock(Math.floor(x), Math.floor(y), Math.floor(z)))
      ) {
        return false;
      }
    }
  ...
  const steerSafe = (
    ...
    if (
      !isSafeDestination(Math.floor(targetX), targetBy, Math.floor(targetZ)) ||
      !isSafeDestination(
        Math.floor(botPos.x + desiredX),
        targetBy,
        Math.floor(botPos.z + desiredZ),
      )
    ) {
      ...
      for (const angle of angles) {
        ...
        if (
          isSafeDestination(Math.floor(testX), targetBy, Math.floor(testZ)) &&
          isSafeDestination(
            Math.floor(botPos.x + rx * mag),
            targetBy,
            Math.floor(botPos.z + rz * mag),
          )
        ) {
  ...
  if (
    ...
    (mode.name.startsWith("/dungeondelver") ||
      mode.name.startsWith("/skycastles"))
  ) {
    for (const id in players) {
      const p = players[id];
      if (p && p.isBot) {
        ...
        if (p.isDead) {
          ...
          if (now - (p.lastDamageTime || 0) > 3000) {
            ...
            let pRespawnData = mode.getRespawnPosition(
              p.id,
              p,
              chunkManager,
              bakedBlocks,
            ...
            while (
              isColumnLava(
                Math.floor(pRespawnData.x),
                Math.floor(pRespawnData.y - 0.1),
                Math.floor(pRespawnData.z),
              ) &&
              retry < 50
            ) {
              pRespawnData = mode.getRespawnPosition(
                p.id,
                p,
                chunkManager,
                bakedBlocks,
        ...
        const currentInLavaColumn =
          isColumnLava(
            Math.floor(p.position.x),
            Math.floor(p.position.y - 0.1),
        ...
        if (currentInLavaColumn) {
          ...
          if (now - (p as any).lavaStuckSince >= timeout) {
            ...
            ioNamespace.emit("chatMessage", {
              sender: "System",
              message: `${p.name} dissolved in lava`,
            });
            broadcastToNearby(
              "playerDied",
              { id: p.id },
        ...
        if ((localBotTickCounter + botOffset) % 15 === 0) {
              ...
              if (cellMobs) {
                for (const m of cellMobs) {
                  if (m.health > 0) {
                    ...
                    const distSq =
                      (m.position.x - p.position.x) ** 2 +
                      (m.position.y - p.position.y) ** 2 +
                      (m.position.z - p.position.z) ** 2;
                ...
                if (cellPlayers) {
                  for (const other of cellPlayers) {
                    if (other.id !== id && !other.isDead) {
                      if (other.team && p.team && other.team === p.team)
                        continue;
                      const distSq =
                        (other.position.x - p.position.x) ** 2 +
                        (other.position.y - p.position.y) ** 2 +
                        (other.position.z - p.position.z) ** 2;
                      if (distSq < closestDist) {
                        closestDist = distSq;
          ...
          if (closestItem && closestType) {
            ...
            if (
              cand &&
              checkLineOfSight(p.position, cand.position) &&
              !hasLavaBetween(p.position, cand.position)
            ) {
        ...
        if ((p as any).targetItem) {
          target =
            (p as any).targetType === "player"
              ? players[(p as any).targetItem]
              : mobs[(p as any).targetItem];
          ...
          if (target && target.position) {
            ...
            if (!isLost && (localBotTickCounter + botOffset + 7) % 15 === 0) {
              isLost =
                !checkLineOfSight(p.position, target.position) ||
                hasLavaBetween(p.position, target.position);
            }
            if (isLost) {
              (p as any).targetItem = null;
              target = null;
        ...
        if (!onGround) {
          ...
          if (
            p.velocity &&
            (Math.abs(p.velocity.x) > 0.01 || Math.abs(p.velocity.z) > 0.01)
          ) {
            const frontBx = Math.floor(
              p.position.x + Math.sign(p.velocity.x) * 0.8,
            );
            const frontBz = Math.floor(
              p.position.z + Math.sign(p.velocity.z) * 0.8,
            );
              ...
              if (target.health > 0) {
                ...
                if ((p as any).targetType === "mob") {
                  ...
                  const isInvulnerable =
                    !mode.name.startsWith("/dungeondelver") &&
                    now - (target.lastRespawnTime || 0) < 5000;
                  if (!isInvulnerable) {
                    ...
                    if (target.health <= 0) {
                      ...
                      if (!target.isDead) {
                        ...
                        ioNamespace.emit("chatMessage", {
                          sender: "System",
                          message: `${target.name || "Unknown"} was slain by ${p.name || "Unknown"}`,
                        });
                        broadcastToNearby(
                          "playerDied",
                          { id: target.id },
                      ...
                      const force = Math.max(
                        ...
                        Math.sqrt(
                          knockbackDir.x * knockbackDir.x +
                            (knockbackDir.y || 0) * (knockbackDir.y || 0) +
        ...
        if (!isKnockedBack && (p.velocity.x !== 0 || p.velocity.z !== 0)) {
          const botOffset =
            (p.id.charCodeAt(0) || 0) + (p.id.charCodeAt(1) || 0);
          if (
            (localBotTickCounter + botOffset) % 10 === 0 ||
            !(p as any).steeredDir
          ) {
            const steered = steerSafe(p.position, p.velocity.x, p.velocity.z);
            const sMag = Math.sqrt(
              steered.x * steered.x + steered.z * steered.z,
            );
            if (sMag > 0.001) {
              (p as any).steeredDir = {
                x: steered.x / sMag,
        ...
        if (
          Math.abs(p.velocity.x) > 0.01 ||
          Math.abs(p.velocity.y) > 0.01 ||
          Math.abs(p.velocity.z) > 0.01
        ) {
          p.isSprinting =
            Math.sqrt(
              p.velocity.x * p.velocity.x + p.velocity.z * p.velocity.z,
  ...
  if (pendingPlayerUpdates.size > 0) {
    ...
    const hoistedPlayerUpdatesByGrid = new Map<
      number,
    ...
    for (const id of pendingPlayerUpdates) {
      ...
      if (p && !p.isDead) {
        ...
        if (
          !mode.name.startsWith("/dungeondelver") &&
          Date.now() - (p.lastRespawnTime || 0) < 5000
        )
          stateMask |= 256;
        ...
        const gridKey = getCellKey(
          Math.floor(p.position.x / PLAYER_CELL_SIZE),
          Math.floor(p.position.z / PLAYER_CELL_SIZE),
        );
    ...
    for (const recipientId in players) {
      ...
      if (nearCount > 0) {
        const size =
          2 + nearCount * 1 + nearIdStrLen + nearCount * 12 * 4 + nearCount * 4;
        let localOffset = 0;
        SHARED_NETWORK_BUFFER.writeUInt16LE(nearCount, localOffset);
        localOffset += 2;

        for (const update of nearUpdates) {
          const idLen = Buffer.byteLength(update.id, "utf8");
          SHARED_NETWORK_BUFFER.writeUInt8(idLen, localOffset);
          localOffset++;
          SHARED_NETWORK_BUFFER.write(update.id, localOffset, idLen, "utf8");
          localOffset += idLen;
        ...
        if (
          recipientSocket.ws &&
          typeof recipientSocket.ws.send === "function"
        ) {
  ...
  if (pendingHits.length > 0) {
    ...
    for (const hit of pendingHits) {
      const gridKey = getCellKey(
        Math.floor(hit.position.x / PLAYER_CELL_SIZE),
        Math.floor(hit.position.z / PLAYER_CELL_SIZE),
      );
