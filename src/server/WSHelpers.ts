...
export function encodePacket(event: string, args: any[]): Buffer | ArrayBuffer {
  ...
  if (
    ...
    const payloadBuf = Buffer.isBuffer(args[0])
      ? args[0]
      : Buffer.from(
          args[0] instanceof ArrayBuffer
            ? args[0]
            : (args[0] as ArrayBufferView).buffer,
        );
...
export function decodePacket(
  ...
  if (typeof data === "string") {
    ...
    } else if (buf.length > 0 && buf[0] < 50) {
      // Binary packet with length prefix
