export class ChatModerator {
  ...
  private playerStates: Map<
    string,
    { lastMessage: string; lastMessageTime: number; messageCount: number }
  > = new Map();
  ...
  constructor() {
    this.badWordRegexes = ChatModerator.BAD_WORDS.map(
      (word) => new RegExp(`\\b${word}\\b`, "i"),
    );
    this.severeWordRegexes = ChatModerator.SEVERE_WORDS.map(
  ...
  public moderateMessage(
    playerId: string,
    message: string,
    options?: { skipSpamCheck?: boolean; skipLinks?: boolean },
    ...
    if (this.hasPII(message)) {
      return {
        isAllowed: false,
        reason: "Sharing personal information or IPs is not allowed.",
      };
    }
    ...
    if (!options?.skipLinks && this.hasLinks(message)) {
      return {
        isAllowed: false,
        reason: "External links and invites are not allowed.",
      };
    }
    ...
    for (const scam of ChatModerator.SCAM_WORDS) {
      if (lowerMsg.includes(scam)) {
        return {
          isAllowed: false,
          reason: "Scam-related messages are prohibited.",
        };
      }
    }
    ...
    for (const regex of this.severeWordRegexes) {
      if (regex.test(aggressiveMessage)) {
        return {
          isAllowed: false,
          reason:
            "Your message violates the community guidelines for severe profanity/hate speech.",
        };
      }
    }
    ...
    if (spacedWords.length > 0) {
      ...
      const collapsedSingles = spacedWords
        .filter((w) => w.length === 1)
        .join("");
