...
export function tickMobs(
  ...
  delta: number,
  now: number,
  fastGetBlock: (x: number, y: number, z: number) => number,
) {
  localMobTickCounter++;
  const {
    ioNamespace,
  ...
  const steerSafe = (
    ...
    if (
      isColumnLava(Math.floor(targetX), targetBy, Math.floor(targetZ)) ||
      isColumnLava(
        Math.floor(mobPos.x + desiredX),
        targetBy,
        Math.floor(mobPos.z + desiredZ),
      )
    ) {
      ...
      for (const angle of angles) {
        ...
        if (
          !isColumnLava(Math.floor(testX), targetBy, Math.floor(testZ)) &&
          !isColumnLava(
            Math.floor(mobPos.x + rx * mag),
            targetBy,
            Math.floor(mobPos.z + rz * mag),
          )
        ) {
  ...
  for (const mId in mobs) {
    ...
    const checkLineOfSight = (p1: any, p2: any) => {
      ...
      for (let i = 1; i <= steps; i++) {
        ...
        if (
          isSolidBlock(
            fastGetBlock(Math.floor(x), Math.floor(y), Math.floor(z)),
          )
        ) {
          return false;
        }
    ...
    if (
      ...
      const mobOffset =
        (mob.id.charCodeAt(0) || 0) +
        (mob.id.charCodeAt(mob.id.length - 1) || 0);
      ...
      if ((localMobTickCounter + mobOffset) % 15 === 0 || !mob.targetId) {
        ...
        if (
          closestPlayerId &&
          checkLineOfSight(mob.position, players[closestPlayerId].position)
        ) {
      ...
      if (mob.targetId && players[mob.targetId]) {
        ...
        if (
          (localMobTickCounter + mobOffset + 7) % 15 === 0 &&
          !checkLineOfSight(mob.position, target.position)
        ) {
          mob.targetId = undefined;
          continue;
        }
        ...
        if (dist > 2.8) {
          if (isLowHealth && dist < 10) {
            ...
            const time = Date.now();
            const seed =
              (mId.charCodeAt(0) || 0) + (mId.charCodeAt(mId.length - 1) || 0);
            ...
            mob.velocity.x +=
              ((dx / dist) * forwardWeight + nx * strafePhase * strafeWeight) *
              speed;
            mob.velocity.z +=
              ((dz / dist) * forwardWeight + nz * strafePhase * strafeWeight) *
              speed;
        ...
        if (dist3D <= 3.5 && Math.random() < 0.25) {
    ...
    const isCurrentlyInLava =
      currentBlock === BLOCK.LAVA ||
      fastGetBlock(bx, Math.floor(mob.position.y + 0.5), bz) === BLOCK.LAVA;
    ...
    if (
      ...
      !isKnockedBack &&
      !inLavaColumn &&
      (mob.velocity.x !== 0 || mob.velocity.z !== 0)
    ) {
      const mobOffset =
        (mob.id.charCodeAt(0) || 0) +
        (mob.id.charCodeAt(mob.id.length - 1) || 0);
      if (
        (localMobTickCounter + mobOffset) % 10 === 0 ||
        !(mob as any).steeredDir
      ) {
    ...
    if (!isMorvane && mob.position.y <= -20) {
      ...
      const frontBx = Math.floor(
        mob.position.x + Math.sign(mob.velocity.x) * 0.8,
      );
      const frontBz = Math.floor(
        mob.position.z + Math.sign(mob.velocity.z) * 0.8,
      );
      ...
      if (isCurrentlyInLava) {
        ...
        const frontBxShore = Math.floor(
          mob.position.x +
            Math.sign(mob.velocity.x || Math.random() - 0.5) * checkDist,
        );
        const frontBzShore = Math.floor(
          mob.position.z +
            Math.sign(mob.velocity.z || Math.random() - 0.5) * checkDist,
        );
        const frontByShore = Math.floor(mob.position.y);
        const blockInFrontShore = fastGetBlock(
          frontBxShore,
          frontByShore,
          frontBzShore,
        );
        const blockInFrontUpShore = fastGetBlock(
          frontBxShore,
          frontByShore + 1,
          frontBzShore,
        );
        if (
          isSolidBlock(blockInFrontShore) ||
          isSolidBlock(blockInFrontUpShore)
        ) {
        ...
        if (
          mob.velocity.y === 0 &&
          Math.abs(mob.position.y - Math.round(mob.position.y)) < 0.05 &&
          isSolidBlock(fastGetBlock(bx, by - 1, bz))
        ) {
        ...
        if (
          Math.abs(mob.velocity.x) > 0.01 ||
          Math.abs(mob.velocity.z) > 0.01
        ) {
    ...
    if (
      ...
      const isWallOrLava = (bx: number, by: number, bz: number) => {
        ...
        if (blk === BLOCK.LAVA) {
          const currentInLavaColumn = isColumnLava(
      ...
      let hitX =
        isWallOrLava(Math.floor(edgeX), nBy, minZ) ||
      ...
      let hitZ =
        isWallOrLava(minX, nBy, Math.floor(edgeZ)) ||
      ...
      if (!hitX && !hitZ) {
        const hitCorner =
          isWallOrLava(Math.floor(edgeX), nBy, Math.floor(edgeZ)) ||
      ...
      if ((hitX || hitZ) && mob.velocity.y <= 0) {
        const currentHeadClear = !isWallOrLava(
          Math.floor(mob.position.x),
          nBy + 2,
          Math.floor(mob.position.z),
        );
    ...
    if (shouldUpdate) {
      ...
      const gridKey = getCellKey(
        Math.floor(x / PLAYER_CELL_SIZE),
        Math.floor(z / PLAYER_CELL_SIZE),
      );
  ...
  if (movedCount > 0) {
    for (const recipientId in players) {
      ...
      if (nearMobCount > 0) {
        ...
        let localOffset = 0;
        localBuf.writeUInt16LE(nearMobCount, localOffset);
        localOffset += 2;

        for (const update of nearMobUpdates) {
          const idLen = Buffer.byteLength(update.mId, "utf8");
          localBuf.writeUInt8(idLen, localOffset);
          localOffset++;
          localBuf.write(update.mId, localOffset, idLen, "utf8");
          localOffset += idLen;
