/**
 * Configuration for game sounds and music.
 * Avoid bundling large audio files (50MB+).
 * Host them on a CDN or External Storage and link the URLs here.
 */

export const SOUND_URLS = {
  // Replace these with your actual hosted URLs
  // Example: 'https://your-cdn.com/audio/bgm_main.mp3'
  bgm_main:
    "https://raw.githubusercontent.com/InventivetalentDev/minecraft-assets/1.19/assets/minecraft/sounds/music/game/creative/creative1.ogg",
  bgm_boss:
    "https://raw.githubusercontent.com/InventivetalentDev/minecraft-assets/1.19/assets/minecraft/sounds/music/game/end.ogg",

  // Existing assets or your custom ones
  ambient_wind:
    "https://raw.githubusercontent.com/InventivetalentDev/minecraft-assets/1.19/assets/minecraft/sounds/ambient/cave/cave1.ogg",
};
