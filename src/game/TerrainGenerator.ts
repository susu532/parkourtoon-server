...
export const biomes = {
  ...
  BADLANDS: {
    height: 25,
    scale: 0.01,
    topBlock: BLOCK.RED_SAND,
    subBlock: BLOCK.TERRACOTTA,
    treeChance: 0.001,
    plantChance: 0.02,
    treeType: "CACTUS",
  },
  ...
  MUSHROOM_ISLAND: {
    height: 10,
    scale: 0.015,
    topBlock: BLOCK.MYCELIUM,
    subBlock: BLOCK.DIRT,
    treeChance: 0.05,
    plantChance: 0.1,
    treeType: "GIANT_MUSHROOM",
  },
  MOUNTAINS: {
    height: 60,
    scale: 0.005,
...
export function getTerrainData(
  ...
  if (isSkyCastles) {
    ...
    if (absZ >= 70 && absZ <= 405) {
      ...
      if (absX > xSlopeStart) {
        xFactor =
          1.0 - Math.min(1.0, (absX - xSlopeStart) / (xSlopeEnd - xSlopeStart));
      ...
      if (absZ >= 170 && absZ <= 230) {
        ...
        baseH += Math.floor(
          Math.max(ridgeNoise, ridgeNoise2) * (1 - peakFactor * 0.4),
        );
    ...
    if (isSkyCastles) {
      ...
      const hullDepth =
        Math.pow(Math.max(0, dVal + noise2D(wx * 0.1, wz * 0.1) * 2), 0.75) *
        4.5;
  ...
  const finalHeight = Math.floor(
    baseHeight * (1 - blendFactor) + targetHeight * blendFactor,
  );
  ...
  if (isSkyCastles) {
    ...
    const hullDepth =
      Math.pow(Math.max(0, dVal + noise2D(wx * 0.1, wz * 0.1) * 2), 0.75) * 4.5;
...
export function getTerrainMinHeight(
  wx_raw: number,
  wz_raw: number,
  isSkyCastles: boolean = false,
) {
