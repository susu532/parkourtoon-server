...
export function createSummerLabTextureAtlas(): THREE.Texture {
  ...
  for (let blockId = 0; blockId < BLOCK_UVS.length; blockId++) {
    ...
    else if (blockId === ItemType.CONCRETE_PINK) color = "#FF00F2";
    else if (blockId === ItemType.CONCRETE_PURPLE) color = "#911FC2";
    else if (blockId === ItemType.CONCRETE_LIME) color = "#32CD32";
    else if (blockId === ItemType.CONCRETE_MAGENTA) color = "#FF00F2";
    else if (blockId === ItemType.CONCRETE_ORANGE) color = "#F07613";
    else if (blockId === ItemType.CONCRETE_LIGHT_BLUE) color = "#3AAfd9";
    else if (blockId === ItemType.CONCRETE_YELLOW) color = "#FFEB13";
    else if (blockId === ItemType.CONCRETE_GRAY)
      color = "#3E4447"; // gray
    else if (blockId === ItemType.CONCRETE_LIGHT_GRAY)
      color = "#8E8E86"; // light gray
    else if (blockId === ItemType.CONCRETE_CYAN)
      color = "#158991"; // cyan
    else if (blockId === ItemType.CONCRETE_BLUE)
      color = "#35399D"; // blue
    else if (blockId === ItemType.CONCRETE_BROWN)
      color = "#724728"; // brown
    else if (blockId === ItemType.CONCRETE_GREEN)
      color = "#546D1B"; // green
    else if (blockId === ItemType.CONCRETE_RED)
      color = "#A12722"; // red
    ...
    for (let face = 0; face < 6; face++) {
      ...
      let hasFace = false;
      if (blockId === ItemType.GLOWSTONE || blockId === ItemType.BLUE_STONE)
        hasFace = true;
      // We can give the kawaii face to "special" interactable blocks
      if (blockId === ItemType.CRAFTING_TABLE || blockId === ItemType.FURNACE)
        hasFace = true;
      ...
      if (tx === 0 && ty === 2) {
        ...
        ctx.moveTo(tx * size + 10, ty * size + 10);
        ctx.lineTo(tx * size + 12, ty * size + 12);
        ctx.stroke();
      } else if (tx === 1 && ty === 2) {
        // Glass
        ctx.clearRect(tx * size, ty * size, size, size);
