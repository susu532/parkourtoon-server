...
export const isWater = (blockType: number) => {
  return (
    ...
    (blockType >= BLOCK.WATER_1 && blockType <= BLOCK.WATER_7)
  );
};
...
export const isGlass = (blockType: number) => {
  return (
    blockType === BLOCK.GLASS ||
    blockType === BLOCK.TINTED_GLASS ||
    (blockType >= BLOCK.GLASS_WHITE && blockType <= BLOCK.GLASS_BLACK)
  );
};
...
export function createTextureAtlas(): THREE.Texture {
  ...
  for (let i = 0; i < 5; i++)
    ctx.fillRect(1 * size + Math.random() * 14, Math.random() * 14, 2, 2);
  ctx.fillStyle = "#ffffff66"; // Tiny white flowers/pebbles
  for (let i = 0; i < 3; i++)
    ctx.fillRect(1 * size + Math.random() * 15, Math.random() * 15, 1, 1);
  ...
  for (let i = 0; i < 2; i++) {
    ...
    ctx.fillRect(cx + 0.5, cy - 0.5, 1, 2);
  }
  // 2,0: Grass side
  ...
  drawTile(
    1,
    1,
    ITEM_COLORS[BLOCK.CRAFTING_TABLE] || "#8f6b42",
    ITEM_COLORS[BLOCK.WOOD] || "#6b4d29",
  );
  ctx.strokeStyle = "#4a3318";
  ctx.beginPath();
  ctx.arc(1.5 * size, 1.5 * size, 4, 0, Math.PI * 2);
  ctx.stroke();
  ...
  ctx.beginPath();
  ctx.moveTo(1 * size + 4, 2 * size + 4);
  ctx.lineTo(1 * size + 6, 2 * size + 6);
  ctx.moveTo(1 * size + 10, 2 * size + 10);
  ctx.lineTo(1 * size + 12, 2 * size + 12);
  ctx.stroke();
  ...
  for (let i = 2; i < 14; i++)
    ctx.fillRect(3 * size + i, 3 * size + 15 - i, 2, 2);
  ...
  for (let i = 0; i < 12; i++) {
    ...
    ctx.fillStyle =
      i % 2 === 0 ? ITEM_COLORS[BLOCK.GRASS] || "#41980a" : "#2d6a07"; // Shaded blades
  ...
  ctx.beginPath();
  ctx.arc(6 * size + 8, 7, 5, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "#ffdf00"; // Bright Petals
  ctx.beginPath();
  ctx.arc(6 * size + 8, 7, 3, 0, Math.PI * 2);
  ctx.fill();
  ...
  for (let i = 0; i < 5; i++)
    ctx.fillRect(
      4 * size + Math.random() * 12,
      1 * size + Math.random() * 16,
      3,
      1,
    );
  ...
  drawTile(
    5,
    1,
    ITEM_COLORS[BLOCK.IRON_SHOVEL] || "#e8e8e8",
    ITEM_COLORS[BLOCK.BIRCH_LOG] || "#d7d7d7",
  );
  ctx.strokeStyle = "#b0b0b0";
  ctx.beginPath();
  ctx.arc(5.5 * size, 1.5 * size, 4, 0, Math.PI * 2);
  ctx.stroke();
  ...
  ctx.beginPath();
  ctx.arc(7.5 * size, 2.5 * size, 6, 0, Math.PI * 2);
  ctx.fill();
  ctx.strokeStyle = "#004400";
  ctx.beginPath();
  ctx.moveTo(7.5 * size - 6, 2.5 * size);
  ctx.lineTo(7.5 * size + 6, 2.5 * size);
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(7.5 * size, 2.5 * size - 6);
  ctx.lineTo(7.5 * size, 2.5 * size + 6);
  ctx.stroke();
  ...
  // Tangled branches
  ctx.beginPath();
  ctx.moveTo(7 * size + 8, 3 * size + 16);
  ctx.lineTo(7 * size + 8, 3 * size + 10);
  ctx.lineTo(7 * size + 4, 3 * size + 6);
  ctx.moveTo(7 * size + 8, 3 * size + 10);
  ctx.lineTo(7 * size + 12, 3 * size + 7);
  ctx.moveTo(7 * size + 6, 3 * size + 8);
  ctx.lineTo(7 * size + 10, 3 * size + 4);
  ctx.stroke();
  ctx.fillStyle = ITEM_COLORS[BLOCK.WOOD] || "#6b4d29";
  ...
  const drawTool = (
    ...
    const rgbToHex = (r: number, g: number, b: number) =>
      `#${((1 << 24) + (Math.max(0, Math.min(255, r)) << 16) + (Math.max(0, Math.min(255, g)) << 8) + Math.max(0, Math.min(255, b))).toString(16).slice(1)}`;
    ...
    for (let i = 0; i < handleLength; i++) {
      ...
      ctx.fillStyle =
        i % 3 === 0 ? handleDark : i % 3 === 1 ? handleMid : handleLight;
      ...
      if (tier >= 2 && i > 2 && i < 6) {
        ctx.fillStyle =
          tier === 4
            ? ITEM_COLORS[BLOCK.DIAMOND_ORE] || "#2ee0d1"
            : tier === 3
              ? ITEM_COLORS[BLOCK.SKYCOIN] || "#ffd700"
              : "#111111";
        ctx.fillRect(px, py, 1, 1);
      }
    }
    ...
    if (isPickaxe) {
      ...
      ctx.fillStyle =
        tier === 0
          ? ITEM_COLORS[BLOCK.WOODEN_PICKAXE] || "#8b5a2b"
          : tier === 1
            ? ITEM_COLORS[BLOCK.STONE] || "#888888"
            : tier === 2
              ? ITEM_COLORS[BLOCK.IRON_PICKAXE] || "#e5e4e2"
              : tier === 3
                ? ITEM_COLORS[BLOCK.SKYCOIN] || "#ffd700"
                : ITEM_COLORS[BLOCK.DIAMOND_PICKAXE] || "#b9f2ff";
      headPixels.forEach(([px, py]) => {
      ...
      ctx.fillStyle = highlight;
      ctx.fillRect(
        x * size + 6 + bladeLength,
        y * size + 8 - bladeLength,
        1,
        1,
      );
  ...
  const drawFood = (x: number, y: number, color: string, isApple: boolean) => {
    ...
    if (isApple) {
      ctx.fillStyle = color;
      ctx.beginPath();
      ctx.arc(x * size + 8, y * size + 9, 6, 0, Math.PI * 2);
      ctx.fill();
  ...
  const drawRod = (x: number, y: number) => {
    ...
    ctx.fillStyle = ITEM_COLORS[BLOCK.WOOD] || "#6b4d29";
    for (let i = 0; i < 14; i++)
      ctx.fillRect(x * size + i, y * size + 15 - i, 2, 2);
    ctx.strokeStyle = "#aaaaaa";
    ctx.beginPath();
    ctx.moveTo(x * size + 13, y * size + 2);
    ctx.lineTo(x * size + 13, y * size + 14);
    ctx.stroke();
  };
  ...
  const drawArrow = (x: number, y: number) => {
    ...
    for (let i = 0; i < 12; i++)
      ctx.fillRect(x * size + i, y * size + 15 - i, 1, 1);
  ...
  const drawPearl = (x: number, y: number) => {
    ...
    ctx.fillStyle = "#111111";
    ctx.beginPath();
    ctx.arc(x * size + 8, y * size + 8, 5, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = ITEM_COLORS[BLOCK.ENDER_PEARL] || "#0b4c42";
    ctx.beginPath();
    ctx.arc(x * size + 8, y * size + 8, 4, 0, Math.PI * 2);
    ctx.fill();
  ...
  const drawBone = (x: number, y: number) => {
    ...
    for (let i = 0; i < 12; i++)
      ctx.fillRect(x * size + i + 2, y * size + 13 - i, 2, 2);
  ...
  drawLump(
    19,
    31,
    ITEM_COLORS[BLOCK.GUNPOWDER] || "#545454",
    "#777777",
    "#333333",
  ); // Gunpowder
  drawLump(
    20,
    31,
    ITEM_COLORS[BLOCK.SNOW] || "#ffffff",
    ITEM_COLORS[BLOCK.SNOW] || "#ffffff",
    ITEM_COLORS[BLOCK.ARROW] || "#dddddd",
  ); // String (bundle)
  drawFeather(21, 31);
  ...
  const drawCoin = (x: number, y: number) => {
    ...
    ctx.beginPath();
    ctx.arc(x * size + 8, y * size + 8, 6.5, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.fillStyle = grad;
    ctx.beginPath();
    ctx.arc(x * size + 8, y * size + 8, 5.5, 0, Math.PI * 2);
    ctx.fill();
  ...
  drawIngot(
    6,
    28,
    ITEM_COLORS[BLOCK.GOLD_ORE] || "#fcee4b",
    "#fff5b1",
    "#bfa51c",
  ); // Gold
  drawGem(
  ...
  drawGem(
    10,
    28,
    ITEM_COLORS[BLOCK.EMERALD_ORE] || "#17dd62",
    "#5fff9d",
    "#0d8a3d",
  ); // Emerald
  drawLump(
    11,
    28,
    ITEM_COLORS[BLOCK.FLOWER_RED] || "#ff0000",
    "#ff5555",
    "#aa0000",
  ); // Redstone
  drawLump(
    12,
    28,
    ITEM_COLORS[BLOCK.LAPIS_ORE] || "#1034bd",
    "#4466ee",
    "#081a5e",
  ); // Lapis
  drawIngot(
    13,
    28,
    ITEM_COLORS[BLOCK.COPPER_INGOT] || "#e39452",
    "#f6b889",
    "#b36a2d",
  ); // Copper Ingot
  ...
  drawTile(
    3,
    9,
    ITEM_COLORS[BLOCK.CONCRETE_LIGHT_BLUE] || "#3aafd9",
    "#2798C1",
  );
  ...
  drawTile(
    8,
    9,
    ITEM_COLORS[BLOCK.CONCRETE_LIGHT_GRAY] || "#8e8e86",
    "#7A7A71",
  );
  ...
  glassColors.forEach((gc, i) => {
    ...
    ctx.beginPath();
    ctx.moveTo(i * size + 4, 11 * size + 4);
    ctx.lineTo(i * size + 6, 11 * size + 6);
    ctx.moveTo(i * size + 10, 11 * size + 10);
    ctx.lineTo(i * size + 12, 11 * size + 12);
    ctx.stroke();
  });
  ...
  for (let i = 0; i < 4; i++)
    ctx.fillRect(
      1 * size + Math.random() * 14,
      6 * size + Math.random() * 14,
      2,
      2,
    );
  ...
  const drawOreExt = (
    ...
    // Base tile
    const c1 = isDeepslate
      ? "#313133"
      : ITEM_COLORS[BLOCK.FURNACE] || "#7d7d7d";
    ...
    if (orePattern === 0) {
    ...
    } else if (orePattern === 1) {
      // Streaks (Iron, Gold, Copper)
      flecks = [
        [2, 2, 3, 2],
        [5, 4, 4, 2],
        [10, 9, 3, 2],
      ...
      // Crystals (Diamond, Emerald, Redstone, Lapis)
      flecks = [
        [4, 4, 3, 3],
        [11, 5, 2, 3],
        [5, 11, 3, 2],
  ...
  for (let i = 0; i < 6; i++)
    ctx.fillRect(
      3 * size + Math.random() * 13,
      22 * size + Math.random() * 13,
      3,
      3,
    );
  ...
  for (let i = 0; i < 8; i++)
    ctx.fillRect(
      5 * size + Math.random() * 12,
      22 * size + Math.random() * 12,
      4,
      4,
    );
  ...
  for (let i = 0; i < 10; i++)
    ctx.fillRect(
      17 * size + Math.random() * 14,
      24 * size + Math.random() * 14,
      2,
      2,
    );
  ...
  const customConcreteSpecs = [
    {
      type: BLOCK.CONCRETE_PASTEL_PINK,
      color: "#FFB6C1",
      shadow: "#E4A0AC",
      x: 16,
      y: 14,
    ...
    {
      type: BLOCK.CONCRETE_PASTEL_PURPLE,
      color: "#E1BEE7",
      shadow: "#C8A9CE",
      x: 21,
      y: 14,
    ...
    {
      type: BLOCK.CONCRETE_NEON_GREEN,
      color: "#39FF14",
      shadow: "#32E012",
      x: 23,
      y: 14,
    ...
    {
      type: BLOCK.CONCRETE_NEON_ORANGE,
      color: "#FF5F1F",
      shadow: "#DF531B",
      x: 24,
      y: 14,
    ...
    {
      type: BLOCK.CONCRETE_NEON_YELLOW,
      color: "#CCFF00",
      shadow: "#B3DF00",
      x: 25,
      y: 14,
    ...
    {
      type: BLOCK.CONCRETE_AQUAMARINE,
      color: "#7FFFD4",
      shadow: "#70DFBA",
      x: 26,
      y: 14,
    ...
    {
      type: BLOCK.CONCRETE_MINT_CREAM,
      color: "#A3E4D7",
      shadow: "#8FC9BE",
      x: 27,
      y: 14,
    ...
    {
      type: BLOCK.CONCRETE_SUNSET_GOLD,
      color: "#FFD700",
      shadow: "#DFBC00",
      x: 29,
      y: 14,
    ...
    {
      type: BLOCK.CONCRETE_SANDY_BEIGE,
      color: "#E5C49F",
      shadow: "#C8AA8B",
      x: 17,
      y: 15,
