...
export function getGiantMythicalShipBlock(
  wx: number,
  wy: number,
  wz: number,
  isBlue: boolean,
  ...
  if (localZ < -30) {
    // Bow taper (sharp)
    currentMaxWidth = Math.max(
      0,
      22 - Math.floor(Math.pow(localZ + 30, 2) / 160),
    );
  } else if (localZ < 30) {
    // Mid body (belly)
    currentMaxWidth =
      22 + Math.floor(Math.sin(((localZ + 30) / 60) * Math.PI) * 4);
  } else {
    // Stern taper (blunt)
    currentMaxWidth = Math.max(
      0,
      22 - Math.floor(Math.pow(localZ - 30, 2) / 100),
    );
  }

  if (localZ > 65)
    currentMaxWidth = Math.max(0, 10 - Math.floor((localZ - 65) * 2)); // Flat-ish back
  ...
  if (localY >= effBottomY && localY <= baseDeckY) {
    ...
    const curveTaper = Math.floor(
      Math.pow(depth / (baseDeckY - effBottomY + 1), 2) * 8,
    );
    ...
    if (w >= 0 && absX <= w) {
      ...
      if (absX >= w) {
        if (localY === baseDeckY || localY === baseDeckY - 1)
          b = secondaryColor;
        ...
        if (localY === 58 + 1 && absX === 0 && localZ % 10 === 0)
          b = BLOCK.GLOWSTONE;
    ...
    if (localY >= 60 && localY <= 65 && localZ > -20 && localZ < 20) {
      const finWidth =
        w + Math.floor(Math.sin(((localZ + 20) / 40) * Math.PI) * 12);
  ...
  if (localZ >= aftBaseZ && localY > baseDeckY) {
    ...
    if (cw >= 0 && absX <= cw) {
      ...
      const isWall =
        absX === cw ||
        localZ === aftBaseZ ||
        (localY > aftDeck1Y ? localZ === aftBaseZ + 10 : false) ||
        localZ > 70;
    ...
    if (
      ...
      if (absX === currentMaxWidth - 2 || localZ === aftBaseZ)
        return secondaryColor;
    }
    if (localY === aftDeck2Y + 1 && localZ >= aftBaseZ + 10) {
      if (absX === currentMaxWidth - 4 || localZ === aftBaseZ + 10)
        return secondaryColor;
    }
  }
  ...
  if (localZ < -80 && localZ > -125) {
    ...
    if (localZ === -90 || localZ === -100) {
      if (localY >= poleY && localY <= poleY + 3 && absX <= 2)
        return primaryColor;
  ...
  for (const mast of mastPositions) {
    ...
    if (
      ...
      if (absX === 4 || localZ === mast.z - 4 || localZ === mast.z + 4)
        return secondaryColor;
      return BLOCK.AIR;
    }
    ...
    const sailTiers = [
      {
        sy: baseDeckY + 10,
        syTop: baseDeckY + Math.min(30, mast.height - 15),
        maxW: 24,
      },
      {
        sy: baseDeckY + 35,
        syTop: baseDeckY + Math.min(50, mast.height - 5),
        maxW: 18,
      },
      {
        sy: baseDeckY + 55,
        syTop: baseDeckY + Math.min(65, mast.height - 2),
        maxW: 12,
      },
    ];
    ...
    for (const tier of sailTiers) {
      if (
        ...
        if (
          localY === tier.syTop - 1 &&
          localZ === mast.z + 1 &&
          (absX === tier.maxW || absX === Math.floor(tier.maxW / 2))
        ) {
          return BLOCK.GLOWSTONE;
        }

        const midY = (tier.sy + tier.syTop) / 2;
        const hBillow = Math.sin(
          ((localY - tier.sy) / (tier.syTop - tier.sy)) * Math.PI,
        );
        const sailZ = mast.z + 1 - Math.floor(hBillow * 8);

        const sailWidth = Math.floor(
          tier.maxW *
            (1 - Math.abs(localY - midY) / ((tier.syTop - tier.sy) * 1.5)),
        );

        if (localZ >= sailZ - 1 && localZ <= sailZ + 1 && absX <= sailWidth) {
          if (absX === sailWidth || localY === tier.sy || localY === tier.syTop)
            return accentColor;
  ...
  if (localY === aftDeck2Y + 1 && localZ === aftBaseZ + 15 && absX <= 1)
    return BLOCK.DARK_OAK_LOG;
  if (localY === aftDeck2Y + 2 && localZ === aftBaseZ + 15 && localX === 0)
    return accentColor;

  if (
    localY === baseDeckY + 1 &&
    (absX === currentMaxWidth || absX === currentMaxWidth - 1) &&
    localZ % 5 === 0 &&
    localZ > -40 &&
    localZ < 30
  ) {
