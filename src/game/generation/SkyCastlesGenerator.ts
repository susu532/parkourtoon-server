...
export function getCastleBlock(
  wx: number,
  wy: number,
  wz: number,
  zOffset: number,
  ...
  queuedMobs: { type: string; pos: THREE.Vector3 }[] | null = null,
): number {
  ...
  const t1 = isTower(-30, -30);
  if (t1 !== -1) return t1;
  const t2 = isTower(-30, 30);
  if (t2 !== -1) return t2;
  const t3 = isTower(30, -30);
  if (t3 !== -1) return t3;
  const t4 = isTower(30, 30);
  if (t4 !== -1) return t4;
  ...
  if (wx >= -31 && wx <= 31 && localZ >= -31 && localZ <= 31) {
    ...
    const isOuterEdgeX =
      (wx === -31 || wx === 31) && localZ > -30 && localZ < 30;
    const isOuterEdgeZ =
      (localZ === -31 || localZ === 31) && wx > -30 && wx < 30;
    ...
    if (isWallX || isWallZ) {
      if (wy <= 15) {
        ...
        if (
          (localZ === frontGateZ || localZ === backGateZ) &&
          wx >= -4 &&
          wx <= 4 &&
          wy <= 10
        ) {
    ...
    if (isOuterEdgeX || isOuterEdgeZ) {
      if (wy <= 14) {
        ...
        if (wy >= 8 && wy <= 13) {
          if (
            (isOuterEdgeX && localZ % 10 === 5) ||
            (isOuterEdgeZ && wx % 10 === 5)
          ) {
            return accentBlock;
          }
        }
  ...
  if (wx >= -14 && wx <= 14 && localZ >= -14 && localZ <= 14) {
    ...
    const kt1 = isKeepTurret(-12, -12);
    if (kt1 !== -1) return kt1;
    const kt2 = isKeepTurret(-12, 12);
    if (kt2 !== -1) return kt2;
    const kt3 = isKeepTurret(12, -12);
    if (kt3 !== -1) return kt3;
    const kt4 = isKeepTurret(12, 12);
    if (kt4 !== -1) return kt4;

    if (wx >= -12 && wx <= 12 && localZ >= -12 && localZ <= 12) {
      const isKeepWall =
        wx === -12 || wx === 12 || localZ === -12 || localZ === 12;
      const dx = wx;
      const dz = localZ;
      const distSq = dx * dx + dz * dz;
      ...
      if (wy <= 40) {
        if (isKeepWall) {
          ...
          if (localZ === keepGateZ && wx >= -3 && wx <= 3 && wy <= 10)
            return BLOCK.AIR;
          ...
          if (wy === 40) {
            ...
            let normalizedStairAngle =
              stairAngle >= 0 ? stairAngle : stairAngle + Math.PI * 2;
            const stepIndex = Math.floor(
              (normalizedStairAngle / (Math.PI * 2)) * 20,
            );
            if (
              distSq > 4 &&
              distSq <= 49 &&
              (stepIndex >= 18 || stepIndex <= 0)
            ) {
              // Let it fall through
            } else {
              return BLOCK.STONE;
          ...
          if (distSq > 4 && distSq <= 49) {
            ...
            const stepIndex = Math.floor(
              (normalizedStairAngle / (Math.PI * 2)) * 40,
            );
      ...
      if (wy > 40 && wy <= 60) {
        ...
        if (distSq <= 100) {
          ...
          if (wy <= 55) {
            if (isTowerWall) {
              // Door to battlements
              if (wy >= 41 && wy <= 43 && dz >= 8 && Math.abs(dx) <= 1)
                return BLOCK.AIR;
