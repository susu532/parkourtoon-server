...
export function getVillageBlock(
  wx: number,
  wy: number,
  wz: number,
  isBlue: boolean,
  isSkyCastles: boolean,
): number {
  ...
  // 1. Village Fence
  const isFenceX =
    (wx === -50 || wx === 50) && absLocalZ >= 0 && absLocalZ <= 49;
  const isFenceZ =
    (localZ === 0 || localZ === (isBlue ? 49 : -49)) && wx >= -50 && wx <= 50;
    ...
    if (dx >= 0 && dx < width && dz >= 0 && dz < depth) {
      ...
      if (wy >= 5 && wy <= 10) {
        if (isRoom) {
          ...
          if (isWall) {
            // Door
            if (dx === 7 && dz === Math.floor(depth / 2) && wy <= 7)
              return BLOCK.AIR;
            // Window
            if (dx === 0 && dz === Math.floor(depth / 2) && wy === 7)
              return BLOCK.GLASS;
            return BLOCK.STONE;
          } else {
    ...
    if (dx >= 0 && dx < width && dz >= 0 && dz < depth) {
      ...
      if (wy >= 5 && wy <= 16) {
        if (dx >= 1 && dx <= width - 2 && dz >= 1 && dz <= depth - 2) {
          const isWall =
            dx === 1 || dx === width - 2 || dz === 1 || dz === depth - 2;
          if (isWall) {
            // Door
            if (dz === 1 && dx === Math.floor(width / 2) && wy <= 7)
              return BLOCK.AIR;
            // Slit windows
      ...
      // Roof
      if (wy === 19 && dx >= 1 && dx <= width - 2 && dz >= 1 && dz <= depth - 2)
        return BLOCK.WOOD;
      if (wy === 20 && dx >= 2 && dx <= width - 3 && dz >= 2 && dz <= depth - 3)
        return BLOCK.WOOD;
    }
    return -1;
  };
    ...
    if (dx >= 0 && dx < width && dz >= 0 && dz < depth) {
      if (wy >= 5 && wy <= 18) {
        ...
        if (isWall) {
          ...
          if (
            dx === 0 &&
            dz >= Math.floor(depth / 2) - 1 &&
            dz <= Math.floor(depth / 2) + 1 &&
            wy <= 7
          )
            return BLOCK.AIR;
          // Windows
          if (wy >= 8 && wy <= 12 && (dx % 3 === 0 || dz % 3 === 0))
            return BLOCK.GLASS;
          return BLOCK.BRICK;
        } else {
  ...
  const drawTownHall = (
    ...
    if (dx >= 0 && dx < width && dz >= 0 && dz < depth) {
      ...
      if (
        wy > 23 &&
        wy <= 30 &&
        dx >= Math.floor(width / 2) - 2 &&
        dx <= Math.floor(width / 2) + 2 &&
        dz >= 2 &&
        dz <= 6
      ) {
        if (wy === 27 && dx === Math.floor(width / 2) && dz === 2)
          return BLOCK.GLASS; // Clock face
        return BLOCK.STONE;
      }
    }
    return -1;
      ...
      if (wy >= 6 && wy <= 9) {
        ...
        if (isStall) {
          ...
          if (
            wy > 6 &&
            wy < 9 &&
            (dx === 2 || dx === 4 || dx === width - 5 || dx === width - 3) &&
            (dz === 2 || dz === 4 || dz === depth - 5 || dz === depth - 3)
          )
            return BLOCK.WOOD;
        }
      }
      ...
      if (distSq <= radiusSq) {
        if (wy >= 5 && wy <= 28) {
          ...
          if (isWall) {
            ...
            // Spiral staircase
            if ((wy + dx) % 4 === 0 && distSq >= radiusSq - 6)
              return BLOCK.WOOD;
            return BLOCK.AIR;
          }
        }
  ...
  for (const b of buildings) {
    ...
    if (b.type === "tavern") res = drawTavern(b.x, bZ, b.w, b.d);
      ...
      res = drawTownHall(b.x, bZ, b.w, b.d, isBlue);
    else if (b.type === "market") res = drawMarket(b.x, bZ, b.w, b.d);
    else if (b.type === "magetower")
      res = drawMageTower(b.x, bZ, b.w, b.d, isBlue);
