...
function getSkyscraperBlock(
  ...
  if (x < cx - 35 || x > cx + 35 || z < cz - 35 || z > cz + 35 || y > maxY)
    return BLOCK.AIR;
  ...
  const isRoof =
    nextWidth === -1 || ((lx > nextWidth || lz > nextWidth) && y !== 9);
  ...
  if (isOuterWall) {
    ...
    const isPillar =
      (lx % 6 === 0 && lz === width) || (lz % 6 === 0 && lx === width);

    if (isCorner) return BLOCK.STONE_BRICKS;

    const pillarBlock =
      seed % 2 === 0 ? BLOCK.DEEPSLATE_BRICKS : BLOCK.QUARTZ_BLOCK;
...
function getWarehouseBlock(
  ...
  const wallColor =
    seed % 2 === 0 ? BLOCK.TERRACOTTA_LIGHT_GRAY : BLOCK.TERRACOTTA_BROWN;

  if (isOuterWall) {
    if (y < 6 && (lx === width - 10 || lx === width - 25) && lz === length)
      return BLOCK.AIR;
...
function getMallBlock(
  ...
  if (y > 10) {
    if (lx < 10 && lz < 10) {
      ...
      if (distSq <= radiusSq && distSq > radiusSq - 20)
        return BLOCK.GLASS_WHITE;
      return BLOCK.AIR;
    }
    return BLOCK.AIR;
  }
...
function getCityBlock(x: number, y: number, z: number): number | null {
  ...
  const distToRoadX =
    (((x % gridSpacing) + gridSpacing + gridSpacing / 2) % gridSpacing) -
    gridSpacing / 2;
  const dRoadX = Math.abs(distToRoadX);

  const distToRoadZ =
    (((z % gridSpacing) + gridSpacing + gridSpacing / 2) % gridSpacing) -
    gridSpacing / 2;
  const dRoadZ = Math.abs(distToRoadZ);

  const isRoad = dRoadX <= halfRoad || dRoadZ <= halfRoad;
  const isSidewalk =
    dRoadX <= halfRoad + sidewalkWidth || dRoadZ <= halfRoad + sidewalkWidth;
  ...
  if (isRoad || isSidewalk) {
    if (y > 0 && y < 6) {
      if (
        dRoadX === halfRoad + sidewalkWidth &&
        dRoadZ === halfRoad + sidewalkWidth
      ) {
  ...
  if (y === 0) {
    if (plotType === "PARK") {
      const px =
        (Math.abs((x % gridSpacing) + gridSpacing) % gridSpacing) -
        gridSpacing / 2;
      const pz =
        (Math.abs((z % gridSpacing) + gridSpacing) % gridSpacing) -
        gridSpacing / 2;
  ...
  if (plotType === "PARK") {
    ...
    if (y === 1 && hashVec(x, 0, z) % 1000 === 0 && distToCenter > 20)
      return BLOCK.CHEST;
