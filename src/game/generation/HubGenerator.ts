...
export function generateHubTerrain(
  chunk: Chunk,
  lx: number,
  lz: number,
  wx: number,
  ...
  for (let y = -60; y <= 0; y++) {
    ...
    if (dist < radiusAtY + noise) {
      ...
      if (y === 0) type = BLOCK.POLISHED_ANDESITE;
        ...
        type =
          Math.sin(wx * 0.5 + wz * 0.5) > 0
            ? BLOCK.COBBLED_DEEPSLATE
            : BLOCK.OBSIDIAN;
      else
        type =
          Math.cos(wx * 0.4 - wz * 0.4) > 0
            ? BLOCK.DEEPSLATE
            : BLOCK.COBBLED_DEEPSLATE;
  ...
  if (dist <= 85) {
    ...
    if (
      (Math.abs(wx) <= 4 && dist <= 85) ||
      (Math.abs(wz) <= 4 && dist <= 85)
    ) {
    ...
    if (dist <= 6) {
      ...
      if (dist <= 2) {
        for (let y = 61; y <= 64; y++)
          chunk.setBlockFast(lx, y, lz, BLOCK.LAVA);
...
export function buildHubCastles(
  chunk: Chunk,
  lx: number,
  lz: number,
  wx: number,
  ...
  for (const [cx, cz, rot] of centers) {
    ...
    if (lx_c >= -width && lx_c <= width && lz_c >= 0 && lz_c <= length) {
      const isBorder =
        Math.abs(lx_c) === width || lz_c === 0 || lz_c === length;
