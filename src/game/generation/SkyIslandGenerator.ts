...
export function generateSkyIslandTerrain(
  chunk: Chunk,
  lx: number,
  lz: number,
  wx: number,
  ...
  for (let y = 0; y < CHUNK_HEIGHT; y++) {
    ...
    if (dist < 40) {
      ...
      const depth = Math.max(
        1,
        15 - dist * 0.35 + noise2D(wx * 0.1, wz * 0.1) * 3,
      );

      if (worldY === baseHeight) {
        chunk.setBlockFast(lx, y, lz, BLOCK.CONCRETE_PINK);
      } else if (worldY < baseHeight && worldY > baseHeight - depth) {
        chunk.setBlockFast(
          lx,
          y,
          lz,
          worldY > baseHeight - 3 ? BLOCK.DIRT : BLOCK.STONE,
        );
      } else if (
        worldY === baseHeight + 1 &&
        dist < 35 &&
        noise2D(wx * 12.3, wz * 12.3) > 0.9
      ) {
        chunk.setBlockFast(lx, y, lz, BLOCK.WOOD);
      } else if (
        worldY > baseHeight &&
        worldY <= baseHeight + 2 &&
        noise2D(wx * 12.3, wz * 12.3) > 0.95
      ) {
    ...
    if (dist2 < 40) {
      ...
      const depth2 = Math.max(
        1,
        15 - dist2 * 0.35 + noise2D(wx2 * 0.1, wz2 * 0.1) * 3,
      );
      ...
      if (worldY === baseHeight2) {
        ...
        chunk.setBlockFast(
          ...
          y,
          lz,
          worldY > baseHeight2 - 3 ? BLOCK.DIRT : BLOCK.STONE,
        );
      } else if (
        worldY === baseHeight2 + 1 &&
        dist2 < 35 &&
        noise2D(wx2 * 12.3, wz2 * 12.3) > 0.9
      ) {
        chunk.setBlockFast(lx, y, lz, BLOCK.WOOD);
      } else if (
        worldY > baseHeight2 &&
        worldY <= baseHeight2 + 2 &&
        noise2D(wx2 * 12.3, wz2 * 12.3) > 0.95
      ) {
        chunk.setBlockFast(lx, y, lz, BLOCK.AIR);
      }
    }
